...
/*   Updated: 2024/08/04 16:48:49 by vodebunm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_usleep(long t, t_philo_arg *philo_av)
{
    ...
    start = get_timeofday(MICROSECOND);
...
void	mutex_write(t_philo_action action, t_philosopher *philo_av, bool debug)
{
	...
	buf = get_timeofday(MILLISECOND) -philo_av->philo_arg->start_activity;
	if (philo_av->satisfy)//subject to check
		return;
